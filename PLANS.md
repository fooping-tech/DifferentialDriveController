...
- [x] (2026-01-01) シリアル送信値をM5Atomの画面に表示する実装を追加する。
...
シリアル送信に使っている左右タイヤ指示値をM5Atomの画面にも表示できるよう、表示レイアウトと更新頻度を決めたうえで、`loop()`内で描画更新を行う。数値が変化したときだけ再描画するなど、画面描画の負荷を抑える方針とする。
...
6. 送信値の画面表示
   - 対象ファイル: `src/main.cpp`
   - 表示API（`display.*`/M5GFX相当）と既存描画コードの使い方を確認する。
   - 画面上の配置を決め、`L`/`R`の数値と符号が一目でわかるレイアウトを設計する。
   - 左右指示値が更新されたときのみ再描画するようにして、描画負荷を抑える。
   - 表示用フォーマット（例: `L:+120`/`R:-080`）を定め、桁や符号の見やすさを優先する。
...
- M5Atomの画面に`L`/`R`の指示値が常時表示され、スティック操作に合わせて更新される。
